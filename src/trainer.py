...
from src.dataset import MRI_Dataset
...
class BaselineClassifier(Trainer):
	...
	def epoch_train(self, model, train_iterator, optimizer, loss_criteria, device): # Epoch training
		...
		model.train()
		training_loss = 0
		for batch, (images, labels) in enumerate(train_iterator):
			# Move X, Y to device
	...
	def set_up_training_data(self):
		...
		train_dataset = MRI_Dataset(self.DATA_DIR_TRAIN_IMAGES, train_data, (self.IMAGE_SIZE, self.IMAGE_SIZE), True)
		...
		val_dataset = MRI_Dataset(self.DATA_DIR_TRAIN_IMAGES, val_data, (self.IMAGE_SIZE, self.IMAGE_SIZE), True)
